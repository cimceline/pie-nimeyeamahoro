...
export default function ResourcesList() {
  ...
  const load = async () => {
    setLoading(true);
    try { const { data } = await resourceAPI.getAll(); setItems(data.resources || data.data || []); } catch {} finally { setLoading(false); }
  };
  useEffect(() => { load(); }, []);

  const openCreate = () => { setEditItem(null); reset({ title: '', category: 'other', description: '' }); setModalOpen(true); };
  const openEdit = (item) => { setEditItem(item); reset(item); setModalOpen(true); };

  const onSubmit = async (data) => {
    try {
      if (editItem) { await resourceAPI.update(editItem._id, data); toast.success('Updated'); }
      else { await resourceAPI.create(data); toast.success('Created'); }
      setModalOpen(false); load();
    } catch (err) { toast.error(err.response?.data?.message || 'Failed'); }
  };

  const handleDelete = async () => {
    try { await resourceAPI.delete(deleteId); toast.success('Deleted'); load(); } catch {} setDeleteId(null);
  };
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-8">
        <div><h1 className="text-2xl font-bold">Resources</h1><p className="text-gray-500">Manage downloadable resources.</p></div>
      ...
      {loading ? <LoadingSpinner className="py-12" /> : (
        <div className="space-y-3">
          {items.length === 0 && <p className="text-gray-500 text-center py-12">No resources.</p>}
          {items.map((item) => (
            <div key={item._id} className="bg-white rounded-xl border p-4 flex items-center justify-between">
                ...
                <p className="text-sm text-gray-500">{item.category}</p></div>
              </div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit Resource' : 'Add Resource'}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div><label className="block text-sm font-medium mb-1">Category</label><select {...register('category')} className="w-full px-3 py-2 border rounded-lg">
            <option value="research_material">Research Material</option><option value="teaching_resource">Teaching Resource</option>
            <option value="report">Report</option><option value="policy_document">Policy Document</option>
            <option value="guide">Guide</option><option value="manual">Manual</option>
            <option value="presentation">Presentation</option><option value="spreadsheet">Spreadsheet</option><option value="other">Other</option>
          </select></div>
      ...
      <ConfirmDialog isOpen={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={handleDelete} title="Delete Resource" message="Are you sure?" confirmText="Delete" />
    </>
  );
}
