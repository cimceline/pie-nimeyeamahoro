...
export default function ProjectsList() {
  ...
  const load = async () => {
    setLoading(true);
    try { const { data } = await projectAPI.getAll(); setItems(data.projects || data.data || []); } catch {} finally { setLoading(false); }
  };
  useEffect(() => { load(); }, []);

  const openCreate = () => { setEditItem(null); reset({ title: '', description: '', status: 'active', url: '' }); setModalOpen(true); };
  const openEdit = (item) => { setEditItem(item); reset(item); setModalOpen(true); };

  const onSubmit = async (data) => {
    try {
      if (editItem) { await projectAPI.update(editItem._id, data); toast.success('Updated'); }
      else { await projectAPI.create(data); toast.success('Created'); }
      setModalOpen(false); load();
    } catch (err) { toast.error(err.response?.data?.message || 'Failed'); }
  };

  const handleDelete = async () => {
    try { await projectAPI.delete(deleteId); toast.success('Deleted'); load(); } catch {} setDeleteId(null);
  };
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-8">
        <div><h1 className="text-2xl font-bold">Projects</h1><p className="text-gray-500">Manage projects.</p></div>
      ...
      {loading ? <LoadingSpinner className="py-12" /> : (
        <div className="space-y-3">
          {items.length === 0 && <p className="text-gray-500 text-center py-12">No projects.</p>}
          {items.map((item) => (
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'Edit Project' : 'Add Project'}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div><label className="block text-sm font-medium mb-1">URL</label><input {...register('url')} className="w-full px-3 py-2 border rounded-lg" /></div>
      ...
      <ConfirmDialog isOpen={!!deleteId} onClose={() => setDeleteId(null)} onConfirm={handleDelete} title="Delete Project" message="Are you sure?" confirmText="Delete" />
    </>
  );
}
